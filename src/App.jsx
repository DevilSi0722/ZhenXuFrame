...
export default function App({ cloud = false, logout }) {
  ...
  useEffect(() => {
    let active = true;
    Promise.all([api('/settings'), api('/jobs')]).then(([config, items]) => { if (active) { setSettings(config); setJobs(items); } }).catch(error => setLoadError(error.message));
    let timer;
  ...
  useEffect(() => {
    if (!cloud) return;
    const update = () => loadJobs().catch(error => setLoadError(error.message));
    window.addEventListener('frame-jobs-changed', update);
    return () => window.removeEventListener('frame-jobs-changed', update);
  }, [cloud, loadJobs]);
  useEffect(() => {
  ...
  async function submit() {
    ...
    } catch (error) { if (error.recoveryJob) { mergeJobs([error.recoveryJob]); setSelectedId(error.recoveryJob.id); } notify(error.message, true); }
  ...
  return <div className="app-shell"><aside className="sidebar"><a href="#" className="brand" onClick={e => { e.preventDefault(); setPage('create'); }}><span className="brand-icon"><img src="/favicon.svg" width="38" height="38" alt="" aria-hidden="true" /></span><div><strong>帧序 <span>FRAME</span></strong><small>VIDEO STUDIO</small></div></a><button className="new-button" onClick={newCreation}><Plus size={17} />新建创作</button><nav aria-label="主导航">{[['create', PanelsTopLeft, '创作工作台'], ['history', HistoryIcon, '任务历史'], ['settings', Settings2, '连接设置']].map(([id, Icon, label]) => <button key={id} aria-label={label} onClick={() => setPage(id)} className={page === id ? 'active' : ''}><Icon size={18} /><span>{label}</span>{page === id && <ChevronRight size={14} />}</button>)}</nav><div className="sidebar-bottom"><a className="docs-link" href="https://doc.deepkey.top/#guide-seedance-video" target="_blank" rel="noreferrer"><ExternalLink size={15} /><span>DeepKey 文档</span></a><button className="connection-tile" onClick={() => setPage('settings')}><span className={`dot ${settings?.hasKey ? 'green' : ''}`} /><div><strong>{settings?.hasKey ? 'API 已配置' : 'API 未配置'}</strong><small>DeepKey · Seedance</small></div><Settings2 size={15} /></button><span className="local-label">{cloud ? '私人工作空间' : '本地工作空间'} <span>v1.1</span></span></div></aside>
    <div className="main-shell"><header className="topbar"><div className="breadcrumb">工作空间<ChevronRight size={13} /><strong>{page === 'create' ? '视频创作' : page === 'history' ? '任务历史' : '连接设置'}</strong></div><span className="topbar-right"><span className="dot green" />{cloud ? '此浏览器存储' : '本地存储'}{cloud && <button className="text-button" onClick={() => logout().catch(error => notify(error.message, true))}>退出登录</button>}</span></header>{loadError && <div role="alert" className="service-error">{loadError}</div>}{cloud && submitting && <div className="submission-note" role="status">正在处理参考图并等待平台返回任务 ID，请暂时保持此页面打开。</div>}
    {page === 'create' ? <main className="studio"><Composer draft={draft} setDraft={setDraft} submit={submit} submitting={submitting} configured={settings?.hasKey} openSettings={() => setPage('settings')} notify={notify} /><div className="workspace">{selected && <button className="text-button back-preview" onClick={() => setSelectedId(null)}><ArrowLeft size={14} />返回灵感预览</button>}<Preview key={selected?.id || 'inspiration'} job={selected} refresh={refresh} reuse={reuse} useSample={useSample} notify={notify} /><RecentJobs jobs={jobs} selectedId={selectedId} select={select} showHistory={() => setPage('history')} /><footer className="workspace-footer"><Clapperboard size={14} /><span>每一帧，都从一个想法开始。</span><span>POWERED BY SEEDANCE</span></footer></div></main> : page === 'history' ? <History browserStorage={cloud} notify={notify} jobs={jobs} select={select} importJob={async remoteId => { try { const job = await post('/jobs/import', { remoteId }); mergeJobs([job]); await loadJobs(); select(job.id); notify('任务已导入'); return true; } catch (error) { notify(error.message, true); return false; } }} /> : settings ? <Settings settings={settings} setSettings={setSettings} notify={notify} /> : <div className="loading-state">{loadError || '正在读取连接设置…'}</div>}
