...
export default function Settings({ settings, setSettings, notify }) {
  ...
  return <main className="settings-page"><div className="page-heading"><div><h1>连接设置</h1><p>DeepKey / Seedance</p></div><a className="secondary" href="https://doc.deepkey.top/#guide-seedance-video" target="_blank" rel="noreferrer">接口文档<ExternalLink size={15} /></a></div>
    {settings.cloud ? <section className="settings-section"><h2><PlugZap size={19} />API 连接</h2><div className="cloud-key-note"><p>API 地址：{settings.baseUrl}</p><p><span className={`dot ${settings.hasKey ? 'green' : ''}`} /> {settings.keySource}</p><p>在 Vercel → Settings → Environment Variables 中设置 <code>DEEPKEY_API_KEY</code>，修改后重新部署。密钥只在服务端使用，不存入浏览器。</p></div><button className="primary" disabled={busy} onClick={() => act(async () => { const config = await api('/settings'); setSettings(config); const result = await post('/connection'); setConnection(result); notify(result.models.length ? '连接成功' : '连接成功，但当前 Key 未开放 Seedance 模型', !result.models.length); })}><PlugZap size={15} />{busy ? '正在测试…' : '测试连接'}</button></section> : <section className="settings-section"><h2><PlugZap size={19} />API 连接</h2><form onSubmit={e => { e.preventDefault(); act(async () => { await save(); notify('连接配置已保存'); }); }}><label htmlFor="base-url"><Link2 size={14} />API 根地址</label><input id="base-url" type="url" readOnly={settings.cloud} required value={baseUrl} onChange={e => { setBaseUrl(e.target.value); setConnection(null); }} /><label htmlFor="api-key"><KeyRound size={14} />API Key</label><div className="key-input"><input id="api-key" type={showKey ? 'text' : 'password'} autoComplete="off" placeholder={settings.hasKey ? '已配置，留空保留当前密钥' : 'sk-…'} value={apiKey} onChange={e => { setApiKey(e.target.value); setConnection(null); }} /><IconButton title={showKey ? '隐藏密钥' : '显示密钥'} onClick={() => setShowKey(!showKey)}>{showKey ? <EyeOff size={17} /> : <Eye size={17} />}</IconButton></div><div className="credential-status"><span className={`dot ${settings.hasKey ? 'green' : ''}`} />{settings.keySource}</div><p className="settings-note">密钥仅由本机后端使用，不写入浏览器存储。手动保存的密钥存放于本地 .local/settings.json；此目录已排除版本控制。</p><div className="settings-actions"><button type="submit" className="primary" disabled={busy}><Save size={15} />保存设置</button><button type="button" className="secondary" disabled={busy} onClick={() => act(async () => { await save(); const result = await post('/connection'); setConnection(result); notify(result.models.length ? `连接成功，可用视频模型 ${result.models.length} 个` : '连接成功，但当前 Key 未开放 Seedance 模型', !result.models.length); })}><PlugZap size={15} />{busy ? '处理中…' : '保存并测试连接'}</button></div></form>
    <div className="key-tools"><button className="text-button" disabled={busy} onClick={() => act(async () => { await save({ useLocalKey: true, apiKey: '' }); notify('已切换到本机 DeepKey 凭据'); })}>使用本机凭据</button><button className="text-button danger" disabled={busy} onClick={() => act(async () => { await save({ clearKey: true, apiKey: '' }); notify('已断开 API 连接'); })}><Unplug size={13} />断开连接</button></div></section>}
